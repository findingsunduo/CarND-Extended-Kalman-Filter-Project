...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) 
{
  ...
  int noise_ax = 9;
  int noise_ay = 9;
  float delta_t = (measurement_pack.timestamp_ - previous_timestamp_) / 1000000.0; //in seconds
  previous_timestamp_ = measurement_pack.timestamp_;

  float dt2 = delta_t * delta_t;
  float dt3 = delta_t * dt2;
  float dt4 = delta_t * dt3;
  //update the F with delta t
  ekf_.F_(0, 2) = delta_t;
  ekf_.F_(1, 3) = delta_t;
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) 
  {
    // TODO: Radar updates
    Tools tl;
    Hj_ = tl.CalculateJacobian(ekf_.x_);
    ekf_.H_ = Hj_;
    ekf_.R_ = R_radar_;
